...
当目标的可展示记录数不超过“爆典每页条数”时，`/爆典 @某人` 会直接发送全部记录；超过上限时只提示总页数，再使用 `/爆典 @某人 页码` 获取指定页。
...
- 合并转发整体计为一条记录，递归保留节点顺序、节点作者、节点内容和原始子转发 ID。
...
- 回复关系：优先使用 QQ 原生回复；原消息失效或平台拒绝时，拆为被回复节点和回复节点
- 媒体缺失：爆典保留其余可读内容，并用 `[图片资源缺失]` 或 `[表情资源缺失]` 占位
...
| `max_nested_forward_depth` | `3`   | 嵌套合并转发深度，范围 1～10   |
...
- 数据按群隔离，JSON 当前使用 Schema v3。
- v1、v2 数据和官方备份仍可读取，并在下一次修改时创建 `.bak` 后惰性升级。
